...
public class EventosListener implements EventListener, MouseListener
{
    ...
    */
    ...
    public void btnBorrarPoco(ActionEvent ae){
        if(vCalc.getPantalla().getText().length()!=0){
            vCalc.setBuffer(vCalc.getPantalla().getText().substring(0, vCalc.getPantalla().getText().length()-1));
        }
    }
    public void btnFactorialActionPerformed(ActionEvent ae){
        vCalc.setBuffer(Double.toString(calc.factorial(calc.getNum1())));
    }
